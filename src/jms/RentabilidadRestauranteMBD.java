...
public class RentabilidadRestauranteMBD implements MessageListener, ExceptionListener  
{
	...
	public RentabilidadRestauranteMBD(TopicConnectionFactory factory, InitialContext ctx) throws JMSException, NamingException 
	{	
...
//		id = new String(md.digest(id.getBytes()));
	...
	private void sendMessage(String payload, String status, Topic dest, String id) throws JMSException, JsonGenerationException, JsonMappingException, IOException
	{
		...
		ExchangeMsg msg = new ExchangeMsg("productss.general.app1", APP, payload, status, id);
		...
		try 
		{
			...
			if(!ex.getSender().equals(APP))
			{
				if(ex.getStatus().equals(REQUEST))
				{
					...
					ListaRentabilidad videos = dtm.getLocalRentabilidad();
					String payload = mapper.writeValueAsString(videos);
					Topic t = new RMQDestination("", "products.test", ex.getRoutingKey(), "", false);
